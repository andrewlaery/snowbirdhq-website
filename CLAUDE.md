...
- **Per-property "Ask Me Anything" AI chat built** (2026-04-23). New route `/docs/properties/{slug}/ask` surfaced as the 5th landing-page nav card. Chat UI in `src/components/property-ask-chat.tsx` (client component, `@ai-sdk/react` `useChat` + `DefaultChatTransport`). API route `src/app/api/chat/[slug]/route.ts` (Node runtime) verifies `docs_portal` / `docs_property` cookies via `verifyCookie`, rate-limits 20 msgs/hr/IP in-memory, loads 4 MDX files via `src/lib/chat/property-context.ts`, streams from **Haiku 4.5** (`claude-haiku-4-5-20251001`) via AI SDK v6 with ephemeral `cacheControl` on the system prompt (min prefix 4096 tokens — all 15 compendiums clear it). `onFinish` logs `cacheReadTokens` / `cacheWriteTokens` / input / output for every exchange. Build + type-check + lint clean locally. Waiting on `ANTHROPIC_API_KEY` in Vercel (Production + Preview via `printf '%s'`, not `echo`) before first real exchange. Preview-deploy first, then prod. See CHANGELOG 2026-04-23.
