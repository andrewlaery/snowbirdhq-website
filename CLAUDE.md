...
**Last session: 2026-05-04** — Website `main` at `005b6e6`, matching `origin/main`.

1. **Docs composition + SOT rollout completed.** All properties are now on the shared composition pattern across EN/ZH/JA. The session included the post-migration cleanup that moved QueenstownEssentials to House Rules, wired PropertyOperationalNotes into Critical Info, added earthquake guidance, removed WiFi passwords from QuickInfo, and fixed markdown rendering in deltas/notes bullets.

2. **Book Direct CTA shipped.** Public marketing header + menu overlay now link to the live Hostaway booking engine at `book.snowbirdhq.com`. Per-property deep-link CTAs remain pending; the URL pattern is `https://book.snowbirdhq.com/listings/{property.hostawayId}`.

3. **3-Kent scaffold/content/brand sweep shipped.** Three 3-Kent docs properties were scaffolded, translated, populated with 21 appliance entries, and branded as "Kent Hill House" / "Upper" / "Lower" across EN/ZH/JA landing pages. Permanent gotcha documented: property `index.mdx` landing pages are hand-written and do not auto-render `identity.yaml::display_name`.

4. **i18n locale switcher fixed.** ZH/JA property listing pages were added, locale switching now preserves property context and no longer hijacks clicks, and the switcher renders for property-cookie users. ZH/JA property landing indexes were aligned to the EN landing template.

**Branches**:
- `__server-vercel/snowbirdhq-website` — local `main` clean except for a pre-existing deleted `.claude/scheduled_tasks.lock`; `main` matches `origin/main` at `005b6e6`.
- Remote stale/old work branches still exist (`feat/i18n-*`, `fix/portfolio-rollout`, `fix/25-dublin-pilot-and-renames`, etc.); inspect before pruning.

**Deployed live**: latest expected production deployment is `005b6e6` via GitHub → Vercel auto-deploy. Public marketing includes Book Direct; docs portal includes composition pattern, 3-Kent content, and locale-switcher fixes.
...
- **CLOSED 2026-05-04: Apply 25-dublin migration pattern to 41-suburb-basecamp + 7-suburb.** Superseded by the full portfolio composition rollout: all 14 properties now use the shared composition pattern across EN/ZH/JA.
...
- **CLOSED 2026-05-04: WiFi credentials in public OTA listing copy.** WiFi credential blocks were scrubbed from the OTA copy files; guest delivery should stay in booking-platform pre-arrival messaging.
...
- **Add per-property Book Direct CTAs** to `/properties/[slug]` using `https://book.snowbirdhq.com/listings/${property.hostawayId}`. Global header/menu CTA shipped 2026-05-04, but the highest-intent property gallery surface still has no booking CTA. (noticed 2026-05-04)
- **Add UTM tagging to Book Direct links** if marketing-to-booking attribution becomes useful (`utm_source=marketing`, placement-specific `utm_medium` / `utm_content`). (noticed 2026-05-04)
- **Add structural-parity check script** to verify all property docs remain on the composition pattern across EN/ZH/JA before merge/deploy. The 2026-05-04 rollout proved manual/multi-agent migration reports are not enough; verify end state file-by-file. (noticed 2026-05-04)
...
- **3-Kent owner-final brand confirmation pending.** "Kent Hill House" / "— Upper" / "— Lower" is Andrew's working draft (2026-04-30); owner approval still TBD. When finalised, update **both** the upstream `identity.yaml::display_name` AND the per-property `content/docs/properties/3-kent-{home,house,apartment}/index.mdx` (EN + ZH + JA) — landing pages are hand-written placeholders, NOT SOT-driven (see "Landing index.mdx pages are not SOT-driven" below). (noticed 2026-05-04)
- **3-Kent pre-go-live checklist** (handover from Goodstays 2026-07-01): (1) Hostaway listing IDs for the 3 listings — currently `'TBC'` in identity.yaml; (2) per-bedroom bed types — currently `bed: TBC`; (3) bathroom counts — currently `bathroom_count: TBC`; (4) `front_door` access narrative for `facts.yaml::exceptions.access` — currently placeholder; (5) photos at `public/properties/3-kent-{home,house,apartment}/` — currently empty; (6) pricing in `facts.yaml::pricing` — currently `0`; (7) cancellation/min-night/instant-bookable verification per listing. All `TBC` markers grep-locatable. (noticed 2026-05-04)
- **Author the SnowbirdHQ supplier register** from 3-Kent supplier list (JAE Cleaning, Queenstown Heatpumps, Aotea Electrical, Remarkables Glass, Glass Earth Cleaning) — these are the inherited Goodstays suppliers for the 3-Kent handover. Should live somewhere shared (probably `__Production/_shared/suppliers/` or similar) so other properties can reference. Source: `_admin/admin-snowbirdhq/bd/prospects/3-Kent/owner-data/2026-04-29-house-instructions.txt` + Goodstays guidebook. (noticed 2026-05-04)
...
## Landing index.mdx pages are NOT SOT-driven (gotcha)

Per-property landing pages (`content/docs/properties/<slug>/index.mdx`) are hand-written MDX placeholders — they do NOT pull from `identity.yaml::display_name` like the welcome / user-instructions / critical-info pages do. They only contain (1) frontmatter title + description and (2) four hardcoded markdown links to the 4 sub-pages.

**Implication when you change a property's brand name**: updating `identity.yaml::display_name` is NOT enough. You also need to update the EN landing index.mdx prose (heading + intro line) AND retranslate to ZH + JA. Verify against the deployed site (auth-gated curl) — the welcome/user-instructions/critical-info pages will already be correct because they render from identity.yaml, so a half-finished brand change shows up only on the index page.

Caught 2026-05-04 when verifying the 3-Kent "Kent Hill House" sweep — sub-pages all rendered the new brand, indexes still said "3 Kent Street — Whole Home / Upstairs / Downstairs". Same shape applies to ANY future brand-name change.
