...
- **AI chat now reads from the SOT** (2026-04-27, commit `0a9a339` on `main`, deployed via Vercel auto-deploy). New module `src/lib/chat/sot-context.ts` loads `identity.yaml` + `facts.yaml` + `guest_copy.md` from `data/sot/properties/<slug>/` and injects them as the primary "PROPERTY FACTS" block in the system prompt; MDX is now supplementary. **`policies.yaml` is intentionally excluded** so master access codes never reach the chat model. Smoke-tested on 7-suburb (47K-char body, no leakage of `access_codes`/master code/`par_levels`). Pre-fix the chat received only the four MDX files, which after Phase 4 are pure `<PropertyWelcome slug=...>`-style component tags — AI had no facts. Title-extraction regex handles apostrophes (`Wright's Garden`).
- **Local `main` synced with origin at `274f947`.** Two further commits landed via auto-commit hook in the same window — `3e23d5b` (7 new appliance components for 25-dublin) and `274f947` (vendored SOT for all 14 properties + 25-dublin MDX cut to composition) — both belong to the ongoing 25-dublin Phase 4 thread, not the chat work.
